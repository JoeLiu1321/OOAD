...
public class Tool {
	...
	public ArrayList<ClassMemberAbstract> getTableValue(JTable temp, ClassMemberAbstract member, String choice) {
		...
		for (int i = 0; i < row; i++) {
			if (choice == "Variable")
				member = new MemberVariable(); // �T�ӫ��O�@��function
			else if (choice == "Function")
				member = new MemberFunction();
			else if (choice == "Relation")
				member = new ClassRelationship();
			else
				return null;
			...
			for (int j = 0; j < column; j++) {
				...
				if (selected != "" && selected != null) {
					if (j == 0)
						member.setReference(selected.toString());
					else if (j == 1 && temp.getColumnName(j) != "Class Name")
						member.setType(selected.toString()); // Class Name���O�I�ssetType
					else
						member.setName(selected.toString());
				}
			}
