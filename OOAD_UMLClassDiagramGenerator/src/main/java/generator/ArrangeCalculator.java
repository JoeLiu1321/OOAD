...
public class ArrangeCalculator {
    ...
    public void removeRelation(Point location) {
        ...
        if (!possibleRelations.isEmpty()) {
            for (Relation relation : possibleRelations) {
                Point2D result = solveEquation(new Point(relation.getStartX(), relation.getStartY()),
                        new Point(relation.getEndX(), relation.getEndY()));
    ...
    public void linkTwoUnitWithMinDistance(Point start, Point end, ClassFormat startClass, ClassFormat endClass,
            Relation relation) {
    ...
    Point2D solveEquation(Point start, Point end) {
        double a = (start.getY() - end.getY()) / (start.getX() - end.getX()),
                b = (end.getY() * start.getX() - start.getY() * end.getX()) / (start.getX() - end.getX());
    ...
    List<Point2D> getPossibleSolution(Point2D equation, ClassFormat classFormat) {
        ...
        int[] value = { classFormat.x, classFormat.y, classFormat.x + classFormat.width,
                classFormat.y + classFormat.height };
